...
# Note the table was padded with dots on all sides.
...
for r,row in enumerate(inputs):
    ...
    for c in range(len(row)):
        if row[c].isdigit():
            ...
            if row[c] in symbols or row[c-width-1] in symbols:
                found_symbol=True
            for local_c in range(c-width-1, c+1):
                if inputs[r-1][local_c] in symbols:
                    found_symbol=True
                if inputs[r+1][local_c] in symbols:
                    found_symbol=True
            if found_symbol:
...
for values in star_location_dict.values():
    if len(values) == 2:
        total+= values[0]*values[1]
    elif len(values) >2:
        print('ERROR: Star is connected to more than two values:')
        for value in values:
            print(value)


#print(star_location_dict)
