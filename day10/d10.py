...
new_table = [' '*len(inputs[0]) for i in range(len(inputs))]
new_table2 = [' '*len(inputs[0]) for i in range(len(inputs))]
...
print(inputs[cur_loc[0]][cur_loc[1]]) 
print(inputs[cur_loc[0]][cur_loc[1]] != "S" )
print(total_length!=0)
no_error=True
while (inputs[cur_loc[0]][cur_loc[1]] != "S" or total_length==0 ) and no_error:
    r = cur_loc[0]
    c = cur_loc[1]
    new_table[r]=  new_table [r][0:c] + inputs[r][c] + new_table[r][c+1:]
    new_table2[r]= new_table2[r][0:c] + '.'          + new_table2[r][c+1:]
    print('cur_loc',cur_loc, inputs[cur_loc[0]][cur_loc[1]])
    #print(inputs[r+1][c]) 
    #print(prior_loc != [r+1,c])
    cur_pipe = inputs[r][c]
    #look right:
    if inputs[r][c+1] in '-J7S' and prior_loc != [r,c+1] and cur_pipe in '-LFS':
        next_loc = [r,c+1]
        if new_table[r][c+2] == ' ' and inputs[r][c+1] == '7':
            new_table[r]=  new_table [r][0:c+1] + '1' + new_table[r][c+2:]
            new_table2[r]= new_table2[r][0:c+1] + '1' + new_table2[r][c+2:]
        if new_table[r][c+2] == ' ' and inputs[r][c+1] == 'J':
            new_table[r]=  new_table [r][0:c+1] + '2' + new_table[r][c+2:]
            new_table2[r]= new_table2[r][0:c+1] + '2' + new_table2[r][c+2:]
        #print('right')
    #look down
    elif inputs[r+1][c] in 'LJ|S' and prior_loc != [r+1,c] and cur_pipe in '|7FS':
        next_loc = [r+1,c]
        if new_table[r+1][c-1] == ' ':
            new_table[r+1]=  new_table [r+1][0:c-1] + '2' + new_table[r+1][c:]
            new_table2[r+1]= new_table2[r+1][0:c-1] + '2' + new_table2[r+1][c:]
        if new_table[r+1][c+1] == ' ':
            new_table[r+1]=  new_table [r+1][0:c+1] + '1' + new_table[r+1][c+2:]
            new_table2[r+1]= new_table2[r+1][0:c+1] + '1' + new_table2[r+1][c+2:]
            #new_table[r]=  new_table [r][0:c+1] + '1' + new_table[r][c+2:]
            #new_table2[r]= new_table2[r][0:c+1] + '1' + new_table2[r][c+2:]
        #print('down')
    #look left
    elif inputs[r][c-1] in '-LFS' and prior_loc != [r,c-1] and cur_pipe in '-7JS':
        next_loc = [r,c-1]
        if new_table[r][c-2] == ' ' and inputs[r][c-1] == 'L':
            new_table[r]=  new_table [r][0:c-2] + '1' + new_table[r][c-1:]
            new_table2[r]= new_table2[r][0:c-2] + '1' + new_table2[r][c-1:]
        if new_table[r][c-2] == ' ' and inputs[r][c-1] == 'F':
            new_table[r]=  new_table [r][0:c-2] + '2' + new_table[r][c-1:]
            new_table2[r]= new_table2[r][0:c-2] + '2' + new_table2[r][c-1:]
        #print('left')
    #look up
    elif inputs[r-1][c] in '7F|S' and prior_loc != [r-1,c] and cur_pipe in '|JLS':
        next_loc = [r-1,c]
        if new_table[r-1][c-1] == ' ':
            new_table[r-1]=  new_table [r-1][0:c-1] + '1' + new_table[r-1][c:]
            new_table2[r-1]= new_table2[r-1][0:c-1] + '1' + new_table2[r-1][c:]
            #new_table[r]=  new_table [r][0:c-1] + '1' + new_table[r][c:]
            #new_table2[r]= new_table2[r][0:c-1] + '1' + new_table2[r][c:]
        if new_table[r-1][c+1] == ' ':
            new_table[r-1]=  new_table [r-1][0:c+1] + '2' + new_table[r-1][c+2:]
            new_table2[r-1]= new_table2[r-1][0:c+1] + '2' + new_table2[r-1][c+2:]
        #print('up')
    else:
        print('error, blocked')
        no_error=False
...
print(total_length)
print("p1:",total_length/2)

    
#P2
for r, row in enumerate(new_table2):
    list_row = [char for char in row] # convert to mutable
    for i in range(len(list_row)):
        if list_row[i] == ' ' and list_row[i-1] == '1':
            list_row[i]='1'
    for i in range(len(list_row)-2,-1,-1):
        #print(list_row)
        #print(len(list_row)-1)
        #print('i',i)
        if list_row[i] == ' ' and list_row[i+1] == '1':
            list_row[i]='1'
    new_table2[r]="".join(list_row)
...
for row in new_table2:
    print(row)

total_num_1s=0
for row in new_table2:
    total_num_1s+=row.count('1')
print("p2: ",total_num_1s)
